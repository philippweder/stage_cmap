...
\begin{equation}
\bigwedge \R^4 := \R \oplus \R^4 \oplus \bigwedge^3\R^4 \oplus \bigwedge^3 \R^4 \oplus \bigwedge^4 \R^4,
\end{equation}
...
%\begin{remark}
%If $\delta p \mid \mid e_{ij}$, then $\star \hat{\omega} = e_{k}$. Consequently, we can take $u := - \sqrt{|\omega|} e_{j}$ to satisfy (\ref{eq:global minimizer condition}). Then we obtain from (\ref{eq:global minimizer form}) that
%\begin{eqnarray}
%	a := -\sqrt{\frac{|\omega|}{2 \pi \mathfrak{g}_k}} \tau_j &,& b := - \sqrt{\frac{|\omega|}{2 \pi \mathfrak{g}_{i}}} \tau_{i}.
%\end{eqnarray}
%Recalling that a net displacement along the $\hat{e}_1$-axis corresponds to $\delta p \mid \mid e_{14}$, the corresponding energy minimizing stroke is an ellipse with respect to the standard euclidean space $(\R^3, (\cdot, \cdot)_{2})$ orthogonal to the vector 
%\end{remark}
