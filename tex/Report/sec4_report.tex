...
We define the so-called \emph{strokes space} as $\dot{H}_\sharp^1(J, \R^4)$, i.e. the Sobolev space of $2\pi$-periodic vector valued functions of $L_{\sharp}^2(J, \R^4)$ having first order weak derivative in $L_{\sharp}^2(\R^4)$ and average zero. Indeed, we can assume a control curve $\xi \in H^1_{\sharp}(J, \R^4)$ without loss of generality since we can add any non-zero average to the initial shape $\xi_0$.
