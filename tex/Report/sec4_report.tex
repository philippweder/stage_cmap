...
\end{align}
Hence, we have in particular the following approximations for any $t \in J $
\begin{align}
...
\end{align}
as well as $ \langle  ( \int_{0}^{t} B_k \dot{\hat{\xi}}(\tau)  \cdot \hat{\xi}(\tau) d \tau  )\dot{\hat{\xi}}_i  \rangle$, $i \in \N_4$ are bounded, where we again set $\xi = \varepsilon \xi$. We focus on the terms of the latter form, since the others can be treated in the same manner. We have
\begin{equation}
\begin{aligned}
...
\begin{align}
  M_k \dot{\xi} \cdot \xi &= - 2 \sqrt{6} \,\alpha( \xi \wedge \dot{\xi} \wedge \tau_{k+1} \wedge \tau_{k+2}), & & k \in \N_3 \\
  M_{3 + k} \dot{\xi} \cdot \xi &= - 2 \sqrt{6} \,\delta  ( \xi \wedge \dot{\xi} \wedge \tau_{k} \wedge \tau_{4}), & & k \in \N_3,
\end{align}
where the indexes are reduce mod 3 to simplify the notation. Ultimately, using that $\R^3 \times \so(3) \simeq \R^6$, we can write the net displacement in position and orientation simultaneously as
\begin{equation}
\delta p = - 4 \pi \sqrt{6} \alpha \sum_{k \in \N_3} ( \xi \wedge \dot{\xi} \wedge \tau_{k+1} \wedge \tau_{k+2}) E_k  - 4 \pi \sqrt{6} \delta \sum_{k \in \N_3} ( \xi \wedge \dot{\xi} \wedge \tau_{k} \wedge \tau_{4}) E_{k + 3},
\end{equation}
where $\{E_k\}_{k \in \N_6}$ denotes the canonical Basis of $\R^6$ and the index $k$ is once more reduced mod 3. This representation will prove particularly useful in the following section.